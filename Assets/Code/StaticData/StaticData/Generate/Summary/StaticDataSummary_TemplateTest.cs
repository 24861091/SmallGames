...
namespace StaticData
{
	...
	{
		private static void Read(XmlNode node ,out StaticData.TemplateTest obj)
		{
			obj = null;
			...
			if (node != null)
			{
				...
				if(obj == null)
				{
					obj = new StaticData.TemplateTest();
				}
				child = XmlSerializerUtility.GetChildrenNode(node, "ID");
				if(child == null)
				{
					obj.ID = 0;
					StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 ID 的XML数据不存在. ID = " + obj.ID);
				}
				else
				{
					...
					{
						obj.ID = 0;
						StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 ID 的XML数据里面是空的,没有写数据.  ID = " + obj.ID);
					}
					else
					{
						if(!System.Int32.TryParse(innerText, out obj.ID))
						{
							StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 ID 的数据格式不对. ID = " + obj.ID);
						}
					}
				}
				...
				{
					obj.a3 = 0;
					StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 a3 的XML数据不存在. ID = " + obj.ID);
				}
				else
				{
					innerText = XmlSerializerUtility.GetInnerText(child);
					...
					{
						obj.a3 = 0;
						StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 a3 的XML数据里面是空的,没有写数据.  ID = " + obj.ID);
					}
					else
					{
						if(!System.Int32.TryParse(innerText, out obj.a3))
						{
							StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 a3 的数据格式不对. ID = " + obj.ID);
						}
					}
				}
				...
				{
					obj.b3 = "";
					StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 b3 的XML数据不存在. ID = " + obj.ID);
				}
				else
				{
					innerText = XmlSerializerUtility.GetInnerText(child);
				...
				{
					obj.c3 = 0;
					StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 c3 的XML数据不存在. ID = " + obj.ID);
				}
				else
				{
					innerText = XmlSerializerUtility.GetInnerText(child);
					...
					{
						obj.c3 = 0;
						StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 c3 的XML数据里面是空的,没有写数据.  ID = " + obj.ID);
					}
					else
					{
						if(!System.Int64.TryParse(innerText, out obj.c3))
						{
							StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 c3 的数据格式不对. ID = " + obj.ID);
						}
					}
				}
				...
				if(child == null)
				{
					...
					StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 platform3 的XML数据不存在. ID = " + obj.ID);
				}
				else
				{
					innerText = XmlSerializerUtility.GetInnerText(child);
					...
					{
						obj.platform3 = default(UnityEngine.RuntimePlatform);
						StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 platform3 的XML数据里面是空的,没有写数据.  ID = " + obj.ID);
					}
					else
					{
						if (!System.Enum.TryParse<UnityEngine.RuntimePlatform>(XmlSerializerUtility.GetInnerText(child), out obj.platform3))
						{
							StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 platform3 的数据格式不对.  ID = " + obj.ID);
						}
					}
				}
				...
				if(child == null)
				{
					...
					StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 v33 的XML数据不存在. ID = " + obj.ID);
				}
				else
				{
					innerText = XmlSerializerUtility.GetInnerText(child);
					...
					{
						obj.v33 = default(UnityEngine.Vector3);
						StaticDataUtility.LogDesinerError("类型为StaticData.TemplateTest 的字段 v33 的XML数据里面是空的,没有写数据.  ID = " + obj.ID);
					}
					else
					{
		...
		private static void Read(BinaryFileReader reader ,out StaticData.TemplateTest obj)
		{
			obj = null;
		...
		private static void Write(XmlDocument doc, XmlNode father,StaticData.TemplateTest obj, string label = "")
		{
