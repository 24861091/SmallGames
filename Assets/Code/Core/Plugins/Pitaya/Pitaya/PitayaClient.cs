...
namespace Pitaya
{
    public class PitayaClient : IDisposable, IPitayaListener
    {
        ...
        public void Request(string route, byte[] msg, Action<string> action, Action<PitayaError> errorAction)
        {
            Request(route, msg, -1, action, errorAction);
        }
        ...
        public void Request<T>(string route, IMessage msg, int timeout, Action<T> action, Action<PitayaError> errorAction)
        {
            ...
            Console.WriteLine($"<color=#FFFF00>[Request]请求发出 route = {route} msg = {msg}</color>");
        ...
        public void Request(string route, byte[] msg, int timeout, Action<string> action, Action<PitayaError> errorAction)
        {
            _reqUid++;
            Action<object> responseAction = res => { action((string)res); };
            ...
            _eventManager.AddCallBack(_reqUid, responseAction, errorAction);
            Console.WriteLine($"<color=#FFFF00>[Request]请求发出 route = {route} msg = {msg}</color>");
            PitayaBinding.Request(_client, route, msg, _reqUid, timeout);
        }
        ...
        public void OnRequestResponse(uint rid, byte[] data)
        {
            ...
            Console.WriteLine($"<color=#FFFF00>[OnRequestResponse]请求结果 rid = {rid} decoded = {decoded}</color>");
            _eventManager.InvokeCallBack(rid, decoded);
        }

        public void OnRequestError(uint rid, PitayaError error)
        {
            Console.WriteLine($"<color=#FFFF00>[OnRequestError]请求报错 route = {rid} error = {error.Code}</color>");
        ...
        public void OnUserDefinedPush(string route, byte[] serializedBody)
        {
            ...
            Console.WriteLine($"<color=#FFFF00>[OnUserDefinedPush]后端推送 route = {route} decoded = {decoded}</color>");
